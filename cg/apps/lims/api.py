...
class LimsAPI(Lims, OrderHandler):
    ...
    def _export_artifact(lims_artifact) -> Dict[str, str]:
        """Get data from a LIMS artifact."""
        return {"id": lims_artifact.id, "name": lims_artifact.name}

    def process(self, process_id: str) -> Process:  # !!!!!!!! # Not used!? Can be deleted!!!!?
        """Get LIMS process."""
    ...
    ## help functions part of cg_lims simply
    ...
    def _most_recent_date(self, dates: list) -> Optional[str]:
        """
    ...
    ### PUT ROUTS
    def update_sample(self, lims_id: str, sex=None, target_reads: int = None, name: str = None, **kwargs) -> None:
    ...
    ### Pool ROUT
    def samples_in_pools(self, pool_name, projectname) -> List[Sample]:  ###!!!!!
        """Fetch all samples from a pool"""
        return self.get_samples(udf={"pool name": str(pool_name)}, projectname=projectname)

    #### Family ROUT
    def family(self, customer: str, family: str) -> Dict[str, Optional]:
    ...
    ## Sample ROUT
    def sample(self, lims_id: str) -> Dict[str, Optional[str]]:
    ...
    def _export_sample(self, lims_sample) -> Dict[str, Optional[str]]:
    ...
    ###All these could be saved as sample level udf
    ...
    ## NOT NEEDED IF SAVED ON SAMPLE????
    @staticmethod
    def _find_capture_kits(artifacts, udf_key: str) -> Set[str]:
        """
    ...
    @staticmethod
    def _find_twist_capture_kits(artifacts, udf_key) -> Set[str]:
        """
    ...
    def _get_methods(self, step_names_udfs, lims_id) -> List[str]:
        """
        ...
        @staticmethod
        def get_method_number(artifact, udf_key_number) -> str:
            """
        ...
        @staticmethod
        def get_method_version(artifact, udf_key_version) -> str:
            """
    ...
    ## Samples filter...
    def get_samples(self, *args, map_ids=False, **kwargs) -> List[Sample]:  ###!!!!!!
